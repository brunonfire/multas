class CarTypesController < ApplicationController
  ...
  def index
    @car_types = CarType.paginate(page: params[:page], per_page: 2)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @car_type.save
        format.html { redirect_to car_type_url(@car_type), notice: create_msg }
  ...
  def update
    respond_to do |format|
      if @car_type.update(car_type_params)
        format.html { redirect_to car_type_url(@car_type), notice: update_msg }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to car_types_url, notice: destroy_msg }
