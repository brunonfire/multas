class FinePointsController < ApplicationController
  ...
  def index
    @fine_points = FinePoint.paginate(page: params[:page], per_page: 2)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @fine_point.save
        format.html { redirect_to fine_point_url(@fine_point), notice: create_msg }
  ...
  def update
    respond_to do |format|
      if @fine_point.update(fine_point_params)
        format.html { redirect_to fine_point_url(@fine_point), notice: update_msg }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to fine_points_url, notice: destroy_msg }
